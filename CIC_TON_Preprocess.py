...
def ReplaceMorethanTenthousandQuantity(df):
    ...
    # df = pd.read_csv(filepath + "\\CIC-ToN-IoT.csv")
    # 获取每个标签的出现次数
    ...
    # 获取所有不同的标签
    unique_labels = df['Attack'].unique()
    ...
    for label in unique_labels:
        ...
        # CIC_TONS如果标签的数量超过5000，提取前5000；否则提取所有行
        if len(label_df) > 5000:
            label_df = label_df.head(5000)
    ...
    # 打印修改后的结果
    print(extracted_df['Attack'].value_counts())
    return extracted_df

# Loading datasets
# dataset = pd.read_csv(filepath + "\\CIC-ToN-IoT.csv")
# dataset = ReplaceMorethanTenthousandQuantity(dataset)
# generatefolder(filepath, "\\dataset_AfterProcessed")
# dataset = clearDirtyData(dataset)
# dataset.to_csv(filepath + "\\dataset_AfterProcessed\\CIC-ToN-IoT_AfterProcessed.csv", index=False)
# 列出每个列的最常见值
...
if(CheckFileExists(filepath + "\\dataset_AfterProcessed\\CIC-ToN-IoT_AfterProcessed.csv")!=True):
    dataset= pd.read_csv(filepath + "\\dataset_AfterProcessed\\CIC-ToN-IoT_AfterProcessed.csv")
    dataset.to_csv(filepath + "\\dataset_AfterProcessed\\CIC-ToN-IoT_AfterProcessed.csv", index=False)
else:
    dataset= pd.read_csv(filepath + "\\dataset_AfterProcessed\\CIC-ToN-IoT_AfterProcessed.csv")
...
label_Encoding('Src IP')
label_Encoding('Src Port')
label_Encoding('Dst IP')
label_Encoding('Dst Port')
...
original_type_values, encoded_type_values = label_encoding("Attack", dataset)
...
# 5000 counts
if(CheckFileExists(filepath + "\\dataset_AfterProcessed\\CIC-ToN-IoT_AfterProcessed_updated_5000.csv")!=True):
    dataset.to_csv(filepath + "\\dataset_AfterProcessed\\CIC-ToN-IoT_AfterProcessed_updated_5000.csv", index=False)
    afterprocess_dataset = pd.read_csv(filepath + "\\dataset_AfterProcessed\\CIC-ToN-IoT_AfterProcessed_updated_5000.csv")

else:
    afterprocess_dataset = pd.read_csv(filepath + "\\dataset_AfterProcessed\\CIC-ToN-IoT_AfterProcessed_updated_5000.csv")

# # 10000 counts
# if(CheckFileExists(filepath + "\\dataset_AfterProcessed\\CIC-ToN-IoT_AfterProcessed_updated_10000.csv")!=True):
#     dataset.to_csv(filepath + "\\dataset_AfterProcessed\\CIC-ToN-IoT_AfterProcessed_updated_10000.csv", index=False)
#     afterprocess_dataset = pd.read_csv(filepath + "\\dataset_AfterProcessed\\CIC-ToN-IoT_AfterProcessed_updated_10000.csv")
...
# 列出要排除的列名
columns_to_exclude = ['Src IP', 'Src Port', 'Dst IP', 'Dst Port', 'Protocol', 'Timestamp']
# 使用条件选择不等于这些列名的列
...
## 重新合並MinMax後的特徵
number_of_components=77 # 原45個的特徵，扣掉'SourceIP', 'SourcePort', 'DestinationIP', 'DestinationPort', 'Protocol', 'Timestamp', 'type' | 84-7 =77
columns_array=[]
...
finalDf = pd.concat([undoScalerdataset,principalDf, afterprocess_dataset[['Attack']]], axis = 1)
# print(finalDf)
...
SaveDataToCsvfile(train_dataframes, f"./TON_IoT Datasets/CIC-ToN-IoT/dataset_AfterProcessed/{today}", f"train_dataframes_{today}")
SaveDataToCsvfile(test_dataframes, f"./TON_IoT Datasets/CIC-ToN-IoT/dataset_AfterProcessed/{today}", f"test_dataframes_{today}")
SaveDataToCsvfile(train_half1, f"./TON_IoT Datasets/CIC-ToN-IoT/dataset_AfterProcessed/{today}", f"train_half1_{today}")
SaveDataToCsvfile(train_half2,  f"./TON_IoT Datasets/CIC-ToN-IoT/dataset_AfterProcessed/{today}", f"train_half2_{today}") 
...
SaveDataframeTonpArray(test_dataframes, f"./TON_IoT Datasets/CIC-ToN-IoT/dataset_AfterProcessed/{today}", "test_CIC_ToN-IoT",today)
SaveDataframeTonpArray(train_dataframes, f"./TON_IoT Datasets/CIC-ToN-IoT/dataset_AfterProcessed/{today}", "train_CIC_ToN-IoT",today)
SaveDataframeTonpArray(train_half1, f"./TON_IoT Datasets/CIC-ToN-IoT/dataset_AfterProcessed/{today}", "train_half1_CIC_ToN-IoT", today)
SaveDataframeTonpArray(train_half2, f"./TON_IoT Datasets/CIC-ToN-IoT/dataset_AfterProcessed/{today}", "train_half2_CIC_ToN-IoT", today)